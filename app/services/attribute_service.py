...
def create_attribute(attribute_data: AttributeCreate, db: Session):
    ...
    try:
        product = db.query(
            Product).filter_by(id=attribute_data.product_id).first()
        if not product:
            raise HTTPException(status_code=400, detail="Товар не найден")
        attribute = db.query(
            Attribute).filter_by(
                name=attribute_data.name, value=attribute_data.value).first()
        if attribute:
            raise HTTPException(
                status_code=400, detail="Характериситика уже существует в бд")
